...
switch ($action) {
  ...
  case 'edited' :
    $json = file_get_contents('https://api.mlab.com/api/1/databases/crma51/collections/gallery/'.$_id.'?apiKey='.MLAB_API_KEY);
...
$url = 'https://api.mlab.com/api/1/databases/crma51/collections/gallery?apiKey='.MLAB_API_KEY.'';
 ...
 function update_field($_id,$field_name,$new_info){
         ...
         $url = 'https://api.mlab.com/api/1/databases/crma51/collections/gallery/'.$_id.'?apiKey='.MLAB_API_KEY;
                 $context = stream_context_create($opts);
                 $returnValue = file_get_contents($url,false,$context);
                 return;
 }
